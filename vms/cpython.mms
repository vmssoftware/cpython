...
.ELSE
CC_QUALIFIERS = $(CC_QUALIFIERS)-
/WARNINGS=(WARNINGS=ALL, DISABLE=(EXTRASEMI,MAYLOSEDATA3,MAYLOSEDATA2,FUNCMIXPTR))
.ENDIF
...
.ELSE
CC_QUALIFIERS = $(CC_QUALIFIERS)-
/POINTER_SIZE=32  ! check 'SIZEOF_VOID_P': 4, in _sysconfigdata__OpenVMS_cpython-310-(ia64|x86_64)-openvms.py
...
CC_FLAGS = $(CC_ONLY_QUALIFIERS)$(CC_QUALIFIERS)/DEFINE=($(CC_DEFINES))/INCLUDE_DIRECTORY=($(CC_INCLUDES))
CXX_FLAGS = $(CC_QUALIFIERS)/DEFINE=($(CC_DEFINES))/INCLUDE_DIRECTORY=($(CC_INCLUDES))

CC_CORE_CFLAGS = $(CC_ONLY_QUALIFIERS)$(CC_QUALIFIERS)/DEFINE=("Py_BUILD_CORE",$(CC_DEFINES))/INCLUDE_DIRECTORY=($(CC_INCLUDES))
CXX_CORE_CFLAGS = $(CC_QUALIFIERS)/DEFINE=("Py_BUILD_CORE",$(CC_DEFINES))/INCLUDE_DIRECTORY=($(CC_INCLUDES))

CC_CORE_MODULE_CFLAGS = $(CC_ONLY_QUALIFIERS)$(CC_QUALIFIERS)/DEFINE=("Py_BUILD_CORE_MODULE",$(CC_DEFINES))/INCLUDE_DIRECTORY=($(CC_INCLUDES))
CXX_CORE_MODULE_CFLAGS = $(CC_QUALIFIERS)/DEFINE=("Py_BUILD_CORE_MODULE",$(CC_DEFINES))/INCLUDE_DIRECTORY=($(CC_INCLUDES))

CC_CORE_BUILTIN_CFLAGS = $(CC_ONLY_QUALIFIERS)$(CC_QUALIFIERS)/DEFINE=("Py_BUILD_CORE_BUILTIN",$(CC_DEFINES))/INCLUDE_DIRECTORY=($(CC_INCLUDES))
CXX_CORE_BUILTIN_CFLAGS = $(CC_QUALIFIERS)/DEFINE=("Py_BUILD_CORE_BUILTIN",$(CC_DEFINES))/INCLUDE_DIRECTORY=($(CC_INCLUDES))

CC_SQLITE3_MODULE_CFLAGS = $(CC_ONLY_QUALIFIERS)$(CC_QUALIFIERS)/DEFINE=("Py_BUILD_CORE_MODULE",MODULE_NAME="""sqlite3""",$(CC_DEFINES))/INCLUDE_DIRECTORY=($(CC_INCLUDES))
CXX_SQLITE3_MODULE_CFLAGS = $(CC_QUALIFIERS)/DEFINE=("Py_BUILD_CORE_MODULE",MODULE_NAME="""sqlite3""",$(CC_DEFINES))/INCLUDE_DIRECTORY=($(CC_INCLUDES))

CC_GETPATH_CFLAGS = $(CC_ONLY_QUALIFIERS)$(CC_QUALIFIERS)/DEFINE=("Py_BUILD_CORE",$(GETPATH_DEFINES))/INCLUDE_DIRECTORY=($(CC_INCLUDES))
CXX_GETPATH_CFLAGS = $(CC_QUALIFIERS)/DEFINE=("Py_BUILD_CORE",$(GETPATH_DEFINES))/INCLUDE_DIRECTORY=($(CC_INCLUDES))
...
############################################################################
# Library
[.$(OUT_DIR)]LIBPYTHON.OLB : [.$(OUT_DIR)]LIBPYTHON.OLB( \
		$(FOREACH FILE, 				\
			$(LIBRARY_OBJS)			\
		, $(BASENAME $(FILE))=$(FILE))	\
		)
...
!    @ pipe create/dir $(DIR $(MMS$TARGET)) | copy SYS$INPUT nl:
!    $(CXX) $(CXX_CORE_MODULE_CFLAGS)/LIST /OBJECT=$(MMS$TARGET) $(MMS$SOURCE)

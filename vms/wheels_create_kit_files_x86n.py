...
--
product VSI X86VMS PYTHWHLS {type}{major}.{minor}-{level}{edit} FULL ;
...
'''
    ...
    kit_content = kit_template.format(
        ...
        dirs='\n    '.join(all_dirs),
        files='\n    '.join(all_files))
    with open('pythwhls_x86n.pcsi$desc', 'w') as file:
        file.write(kit_content)

    text_template = '''=product VSI X86VMS PYTHWHLS {type}{major}.{minor}-{level}{edit} full
1 'PRODUCT
...
'''
    text_content = text_template.format(
        ...
        dirs='\n    '.join(all_dirs),
        files='\n    '.join(all_files))
    with open('pythwhls_x86n.pcsi$text', 'w') as file:
...
if __name__ == "__main__":
    ...
    minor = '1'
    level = '6'
    edit = 'fix07'   # 'd' + datetime.date.today().strftime('%Y%m%d')
